...
## Overview
The frontend of the project was built as a set of dedicated HTML pages, each one corresponding to a specific part of the application: landing page, authentication flow, profile, lobby, friends page, leaderboard, history pages, and terms of service...

The HTML files define the structure of each page: sections, buttons, forms, links, cards, profile areas, and game containers. The visual identity, responsive behavior, and reusable UI elements are handled through a Tailwind-based CSS workflow combined with custom styles.
...
This setup was chosen to keep the frontend both simple to use and structured enough for a team project.

Using dedicated HTML pages keeps the application easy to understand.  
Using Tailwind as part of the build pipeline allows us to keep a clean CSS workflow.  
Separating global tokens from components makes the interface easier to extend and modify without duplicating styles.

Overall, this frontend organization helps maintain consistency across the project while keeping development practical for multiple contributors.
